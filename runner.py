...
BASE_ARGS = [
    ...
    "--idle_data_coherence_time_sec", "10.0",
    "--idle_comm_coherence_time_sec", "10.0",
...
def run_tasks(tasks: list[list[str]], parallel: int = 12) -> None:
...
def main_distance_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--css_code", "[[7,1,3]]", "--target_fidelity", "0.8", "--num_logical_pairs", "30", "--gate_fidelity", "1.0", "--two_qubit_gate_fidelity", "1.0", "--idle_data_coherence_time_sec", "1000000000000.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--ft_prep_mode", "minimal", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/distance_sweep"]
...
def main_two_qubit_gate_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--css_code", "[[7,1,3]]", "--target_fidelity", "0.8", "--num_logical_pairs", "100", "--link_distance_km", "10.0", "--gate_fidelity", "1.0", "--idle_data_coherence_time_sec", "1000000000000.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--ft_prep_mode", "minimal", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--run_duration_ms", "100000.0", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/two_qubit_gate_sweep"]
...
def main_ft_mode_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--css_code", "[[7,1,3]]", "--target_fidelity", "0.8", "--num_logical_pairs", "30", "--link_distance_km", "10.0", "--gate_fidelity", "1.0", "--two_qubit_gate_fidelity", "1.0", "--idle_data_coherence_time_sec", "1000000000000.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--run_duration_ms", "100000.0", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/ft_mode_sweep"]
...
def main_css_code_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--target_fidelity", "0.8", "--num_logical_pairs", "30", "--link_distance_km", "10.0", "--gate_fidelity", "1.0", "--two_qubit_gate_fidelity", "1.0", "--idle_data_coherence_time_sec", "1000000000000.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--ft_prep_mode", "minimal", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--run_duration_ms", "100000.0", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/css_code_sweep"]
...
def main_distance_twoq_code_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--target_fidelity", "0.8", "--num_logical_pairs", "200", "--gate_fidelity", "1.0", "--idle_data_coherence_time_sec", "1000000000000.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--ft_prep_mode", "minimal", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/distance_twoq_code_sweep"]
...
def main_graph1_correction_mode_compare() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = BASE_ARGS + ["--config_file", "config/standard_configs/line_6_2G.json", "--num_logical_pairs", "200", "--log_directory", "log/runner/correction_mode_compare"]

    two_qubit_gate_fidelities = ["0.995", "0.997", "0.998", "0.999", "0.9995", "0.9999", "1.0"]
...
def main_graph2_data_coherence_sweep() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--css_code", "[[7,1,3]]", "--target_fidelity", "0.8", "--num_logical_pairs", "200", "--link_distance_km", "10.0", "--gate_fidelity", "1.0", "--two_qubit_gate_fidelity", "1.0", "--idle_comm_coherence_time_sec", "1000000000000.0", "--ft_prep_mode", "minimal", "--idle_pauli_x", "0.05", "--idle_pauli_y", "0.05", "--idle_pauli_z", "0.9", "--run_duration_ms", "700000.0", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/data_coherence_sweep"]

    idle_data_coherence_times = [0.01, 0.05, 0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0, 500, 1000.0]  # Sweep only data-memory coherence time in seconds.
    # idle_data_coherence_times = [0.05, 0.5, 500.0]  # Sweep only data-memory coherence time in seconds.

    correction_modes = ["none", "cec", "qec"]
    for idle_data_coherence_time_sec in idle_data_coherence_times:
        for correction_mode in correction_modes:
            args = ["--idle_data_coherence_time_sec", str(idle_data_coherence_time_sec), "--correction_mode", correction_mode]
            tasks.append(command + base_args + args)
...
def main_graph3_distance_sweep() -> None:
    ...
    for config_file, run_duration_ms in config_duration_pairs:
        base_args = BASE_ARGS + ["--config_file", config_file, "--run_duration_ms", str(run_duration_ms), "--log_directory", "log/runner/graph3_distance_sweep"]
        for correction_mode in correction_modes:
...
def main_graph4_link_count_sweep() -> None:
    ...
    for config_file, run_duration_ms in config_duration_pairs:
        base_args = BASE_ARGS + ["--config_file", config_file, "--run_duration_ms", str(run_duration_ms), "--log_directory", "log/runner/graph4_link_count_sweep"]
        for correction_mode in correction_modes:
            args = ["--correction_mode", correction_mode]
            tasks.append(command + base_args + args)
...
def main_two_qubit_qec_isolation() -> None:
    ...
    base_dir = Path(__file__).resolve().parent
    command = [sys.executable, str(base_dir / "main.py")]
    base_args = ["--config_file", "config/standard_configs/line_6_2G.json", "--css_code", "[[7,1,3]]", "--target_fidelity", "0.8", "--num_logical_pairs", "200", "--link_distance_km", "10.0", "--gate_fidelity", "1.0", "--measurement_fidelity", "1.0", "--idle_data_coherence_time_sec", "1e12", "--idle_comm_coherence_time_sec", "1e12", "--ft_prep_mode", "minimal", "--run_duration_ms", "100000.0", "--round_spacing_ms", "1.0", "--log_directory", "log/runner/two_qubit_qec_isolation"]
