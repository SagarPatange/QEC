...
def main_distance_and_link() -> None:
    """Answer the question: for a distance, what number of links is optimal? Assuming z=0.9
    """
    tasks: list[list[str]] = []
    base_dir = Path(__file__).resolve().parent
    ...
    config_files_pairs = [
            ...
            # ("config/standard_configs/line_3_2G.json", "10000"),
    ...
    z_value = 0.9
    # 8 configurations x 13 distances = 104 tasks
    # minus line_2 & line_3 for distance > 100 (10 tasks) = 94 tasks
    for config_file, pairs in config_files_pairs:
        ...
        for total_end_to_end_distance_km in total_end_to_end_distance_km_list:
            ...
            if topology in ["line_2"] and total_end_to_end_distance_km > 100:
                continue
            args = BASE_ARGS + [
                    "--config_file", config_file,
                    "--link_distance_km", str(inter_node_distance_km),
                    "--log_directory", "log/runner_distance_and_link",
    ...
    run_tasks(tasks, parallel=50)



if __name__ == "__main__":

    main_distance_and_link()
